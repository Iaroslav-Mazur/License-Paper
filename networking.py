...
blockchain_lock = threading.Lock() #the lock that must be acquired before/released after each operation involving blockchain
...
# print("Genesis block: ", blockchain[0])
			...
			if len(active_peers_ports) < MAX_ACTIVE_CONNECTIONS_IN:
				...
				if addr_me == my_port_nr:
					...
					if ver_ack == bytes((str(my_version_msg[0]) + 'accepted').encode('UTF-8')):
						...
						print('Connected by {}'.format(addr_you))
					else:
				...
				if find_sock_out(addr_you) is None:
					...
					if type(connection_result) is tuple:
						...
						# print("Appended port {} from inside talk_to_a_client".format(addr_you))
						remember_peers()
					else:
				...
				# 		print("The current difference is: your_highest_block({}) - current_highest_block({}) = {}".format(your_highest_block, current_highest_block, difference))
				# 		for i in range(1, difference+1):
				# 			peers_socks_vers_out_lock.acquire() #potential problem here
				# 			print("acquired peers_socks_vers_out_lock before syncing with {} in talk_to_a_client".format(addr_you))
				# 			s.sendall(b"get_block")
				# 			print("sent get_block to {}".format(addr_you))
				# 			temp = s.recv(1024)
				# 			if temp != b"get_block":
				# 				print("Haven't received the correct echo of b\"get_block\"")
				# 				STATE_CATCHING_UP = False
				# 				peers_socks_vers_out_lock.release()
				# 				break
				# 			print("received the correct echo of get_block from {}".format(addr_you))
				# 			wanted_block_nr = current_highest_block + i
				# 			s.sendall(pickle.dumps(wanted_block_nr))
				# 			print("Asked {} to send me the block {}".format(addr_you,wanted_block_nr))
				...
				# 				print("The parent block at the height of {} is already present!".format(wanted_block_nr))
				# 			else:
				# 				blockchain.insert(wanted_block_nr, block_received)
				# 				print("Added block {} to the blockchain!".format(wanted_block_nr))
				# 		if STATE_CATCHING_UP != False:
							# peers_socks_vers_out_lock.acquire()
						# 	s.sendall(b"reveal_your_highest_block")
						# 	print("Asked {} to tell me his highest block".format(addr_you,))
				...
				print("{}, I'm full. Try connecting to my peers".format(addr))
				pickled_peers = pickle.dumps(active_peers_ports)
				conn.sendall(pickled_peers)
				#end the thread from within
				return
		else:
	...
	while True:
		...
		if data == b'get_peer_ports':
			...
			if STATE_CATCHING_UP is False:
				...
				print("Received block {} with hash {}".format(new_block_id,new_block.get_hash_hex()))
				blockchain_lock.acquire()
...
def react_to_take_new_block(blockchain, blockchain_lock, new_block_id, new_block):
	if len(blockchain) == new_block_id: #accept just the direct child of the current highest block
		...
		if miner.is_block_valid(new_block, parent_block) is True:
			...
			print("Added the received block {}  with hash {} to the blockchain".format(new_block_id, new_block.get_hash_hex()))
		else:
			print("the new block, {}, is invalid".format(new_block_id))
		...
		if new_block.block_header.prev_block_hash == current_latest_block.block_header.prev_block_hash:
			if miner.is_block_valid(new_block, parent_block) is True:
				...
				if new_block.block_header.nonce > current_latest_block.block_header.nonce:
					...
					print("Replaced the latest block {} with hash {} with a new one that's been more difficult to mine with the hash {}".format(new_block_id, current_latest_block.get_hash_hex(), new_block.get_hash_hex()))
					...
					if int(new_block_hash, 16) < int(current_latest_block_hash, 16):
						blockchain[new_block_id] = new_block
						#help propagate the replaced block
						print("Replaced the latest block {} with hash {} with a new one (hash: {}) with the same nonce, but smaller (int-wise) hash".format(new_block_id, current_latest_block.get_hash_hex(), new_block.get_hash_hex()))
	else:
		print("The new block, {}, isn't the direct child or grandchild of block {}".format(new_block_id, len(blockchain)-1))
	blockchain_lock.release()
...
def connect_to_a_peer(peers_port):
	try:
		...
		if data == b'version_msg':
			...
			print("Connected to {}".format(peers_port))
			...
			global STATE_CATCHING_UP
			if your_highest_block > current_highest_block and STATE_CATCHING_UP == False:
				print("detected that the peer {} has more blocks than me".format(addr_you))
				STATE_CATCHING_UP = True
				# print("Set STATE_CATCHING_UP to True")
				blockchain_lock.acquire()
				# print("acquired blockchain_lock for syncing with {}".format(addr_you))
				while your_highest_block > current_highest_block:
					difference = your_highest_block - current_highest_block
					print("The current difference is: your_highest_block({}) - current_highest_block({}) = {}".format(your_highest_block, current_highest_block, difference))
					for i in range(1, difference+1):
				...
				try:
					...
					print("Exception when trying to release blockchain_lock!")
				STATE_CATCHING_UP = False
		...
		print("Couldn't connect to {}. Like, at all!".format(peers_port))
		print(e)
		...
		try:
			...
			print("Exception when trying to release blockchain_lock in finally!")
	...
	while len(ports_to_be_tried)>0:
		...
		if len(peers_socks_vers_out) < MAX_ACTIVE_CONNECTIONS_OUT:
			...
			if port_nr != my_port_nr and find_sock_out(port_nr) is None:
				...
				if type(connection_result) is tuple:
					peers_socks_vers_out.append(connection_result)
					# print("Appended port {} from inside connect_to_more_peers".format(port_nr))
					remember_peers()
					...
					print("Couldn't connect to a peer: something went wrong!")
...
def get_peer_ports(s):
	...
	# print("Sent get_peer_ports to {}".format(s))

	data = s.recv(1024)
	# print("Received peer_ports from {}".format(s))
	peers_ports = pickle.loads(data)
	shuffle(peers_ports)
	...
	def check_whether_alive():
		try:
			for sock_ver in peers_socks_vers_out[:]:
				...
				# print("Time when sending 'still alive' to {}: {}".format(sock_ver[1][2], str(time.time())))
				try:
					...
					if reply != b'still alive':
						...
						if sock_ver[1][2] in failed_still_alives:
							...
							if failed_still_alives[sock_ver[1][2]] > 5:
								print ("{} is no longer alive!".format(sock_ver[1][2]))
					...
					if sock_ver[1][2] in failed_still_alives:
						...
						if failed_still_alives[sock_ver[1][2]] > 5:
							print ("\nsocket.timeout for {}!\n".format(sock_ver[1][2]))
							# print("Time when socket.timeout for {}".format(sock_ver[1][2], str(time.time())))
							shutdown_close_remove(sock_ver)
							#remember just the active peers
							remember_peers()
					else:
			...
			try:
				...
				# print("Exception when trying to release peers_socks_vers_out_lock in finally!")
				pass
			...
			if previous_active_peers_ports != current_active_peers_ports:
				if len(peers_socks_vers_out) < MAX_ACTIVE_CONNECTIONS_OUT:
					...
					for sock_ver in shuffled_peers_socks_vers_out:
						# print("Trying to get peers from {}".format(sock_ver[0]))
...
def print_blockchain_state():
	...
	while True:
		...
		if new_bc_len > current_bc_len:
			print("Right now, the blockchain contains {} blocks (excluding the Genesis one):".format(len(blockchain)-1))
			for i in range(len(blockchain)):
				block = blockchain[i]
				print("Block {} hash: {}".format(i,block.get_hash_hex()))
	...
	try:
		...
		while True:
			...
			if new_highest_block_nr > current_highest_block_nr:
					...
					try:
						for sock_ver in peers_socks_vers_out[:]:
							...
							if reply != b'take_new_block':
								print ("Couldn't forward the new block {} to {}. It hasn't echoed \"take_new_block\"!".format(i, sock_ver[1][2]))
							else:
								sock_ver[0].sendall(tuple_to_share)
								print("Sent block {} with hash {} to {}".format(i, new_block.get_hash_hex(), sock_ver[1][2]))
					except:
						print ("Something went wrong when trying to forward the new block {} to a peer!".format(i))
				...
				# print("new_highest_block_nr: {}, current_highest_block_nr: {} (must be equal!)".format(new_highest_block_nr,current_highest_block_nr))
				...
				for sock_ver in peers_socks_vers_out[:]:
					try:
						...
						if reply != b'take_new_block':
							...
							sock_ver[0].sendall(tuple_to_share)
							print("Sent the replaced block {} with hash {} to {}".format(current_highest_block_nr, replaced_block.get_hash_hex(), sock_ver[1][2]))
							# print("parent_block hash: {}\nreplaced_block hash: {}".format(parent_block.get_hash_hex(), replaced_block.get_hash_hex()))
					except socket.timeout:
						pass
				peers_socks_vers_out_lock.release()
	...
	if go_for_the_seed == 1:
		if my_port_nr != SEED_NODE_PORT:
			if find_sock_out(first_peer_port_nr) is None:
				...
				if type(connection_result) is tuple:
					print("Appended port {} from main".format(first_peer_port_nr))
					s = connection_result[0]
					...
					print("Connection to first_peer_port_nr has failed!")
